...
export async function runOnce() {
    const stockSignals = await runStocksOnce(STOCK_UNIVERSE);
    const cryptoSignals = await runCryptoOnce(CRYPTO_UNIVERSE);
...
function upsertSignal(signal) {
    ...
    const row = selectSignalIdStmt.get(signal.uniq_key);
    return row.id;
}
function enqueueForTiers(signalId, signal) {
...
function selectSignals(candidates) {
    const now = Math.floor(Date.now() / 1000);
  ...
  `).all();
    ...
    for (const row of lastSentRows) {
        if (row.last_ts)
            lastSentMap.set(row.symbol, row.last_ts);
    }
