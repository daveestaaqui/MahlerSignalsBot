...
export function score(asset) {
    const norm = (value, scale = 1) => Number.isFinite(value) ? Math.tanh(value / scale) : 0;
...
function tierFor(scoreValue) {
    if (scoreValue >= ELITE_THRESHOLD)
        return 'ELITE';
    if (scoreValue >= PRO_THRESHOLD)
        return 'PRO';
    return 'PRO';
}
function buildSummary(asset, scoreValue) {
...
export async function selectTop(nPro = 20, nElite = 10) {
