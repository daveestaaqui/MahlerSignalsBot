const SQL_INSERT_USER = normalizeSql("INSERT INTO users (id, tier) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET tier=excluded.tier");
const SQL_SELECT_USER_TIER = normalizeSql("SELECT tier FROM users WHERE id=?");
const SQL_SELECT_SNAPSHOT = normalizeSql("SELECT payload, fetched_at, ttl_seconds FROM data_snapshots WHERE source = ? AND asset = '*'");
const SQL_UPSERT_SNAPSHOT = normalizeSql("INSERT INTO data_snapshots (source, asset, fetched_at, payload, ttl_seconds) VALUES (?, '*', ?, ?, ?) ON CONFLICT(source, asset) DO UPDATE SET fetched_at = excluded.fetched_at, payload = excluded.payload, ttl_seconds = excluded.ttl_seconds");
const SQL_INSERT_SIGNAL = normalizeSql("INSERT INTO signals (symbol, asset_type, tier_min, score, reason, features, created_at, embargo_until, uniq_key) VALUES (@symbol, @asset_type, @tier_min, @score, @reason, @features, @created_at, @embargo_until, @uniq_key) ON CONFLICT(uniq_key) DO UPDATE SET score = excluded.score, reason = excluded.reason, features = excluded.features, created_at = excluded.created_at, embargo_until = excluded.embargo_until");
const SQL_SELECT_SIGNAL_ID = normalizeSql("SELECT id FROM signals WHERE uniq_key = ?");
const SQL_INSERT_QUEUE = normalizeSql("INSERT INTO publish_queue (signal_id, tier, payload, ready_at, sent_at, attempts, last_error) VALUES (@signal_id, @tier, @payload, @ready_at, NULL, 0, NULL) ON CONFLICT(signal_id, tier) DO UPDATE SET payload = excluded.payload, ready_at = excluded.ready_at, sent_at = NULL, attempts = 0, last_error = NULL");
const SQL_SELECT_QUEUE_READY = normalizeSql("SELECT pq.id, pq.signal_id, pq.tier, pq.payload, pq.attempts, s.features FROM publish_queue pq JOIN signals s ON s.id = pq.signal_id WHERE pq.ready_at <= ? AND pq.sent_at IS NULL ORDER BY pq.ready_at ASC LIMIT ?");
const SQL_MARK_SUCCESS = normalizeSql("UPDATE publish_queue SET sent_at = ?, attempts = attempts + 1, last_error = NULL WHERE id = ?");
const SQL_MARK_FAILURE = normalizeSql("UPDATE publish_queue SET attempts = attempts + 1, last_error = ?, ready_at = ready_at + 600 WHERE id = ?");
const SQL_UPSERT_METRIC = normalizeSql("INSERT INTO signal_metrics(signal_id, tier, entry_price, sent_at) VALUES(?,?,?,?) ON CONFLICT(signal_id, tier) DO UPDATE SET entry_price=excluded.entry_price, sent_at=excluded.sent_at");
const SQL_WEEKLY_SUMMARY = normalizeSql("SELECT pq.tier, pq.sent_at, s.symbol, s.score, s.features, m.entry_price, m.exit_price_1d, m.exit_price_3d FROM publish_queue pq JOIN signals s ON s.id = pq.signal_id LEFT JOIN signal_metrics m ON m.signal_id = pq.signal_id AND m.tier = pq.tier WHERE pq.sent_at IS NOT NULL AND pq.sent_at >= ?");
const SQL_LAST_SENT = normalizeSql("SELECT s.symbol, s.asset_type, MAX(pq.sent_at) as last_sent FROM publish_queue pq JOIN signals s ON s.id = pq.signal_id WHERE pq.sent_at IS NOT NULL GROUP BY s.symbol, s.asset_type");
const SQL_POSTED_TODAY = normalizeSql("SELECT s.asset_type, COUNT(DISTINCT pq.signal_id) as cnt FROM publish_queue pq JOIN signals s ON s.id = pq.signal_id WHERE pq.sent_at IS NOT NULL AND pq.sent_at >= ? GROUP BY s.asset_type");
const SQL_SELECT_LEDGER_COUNTS = normalizeSql("SELECT asset_class, count FROM publish_ledger WHERE ledger_date = ?");
const SQL_SELECT_LEDGER_SINGLE = normalizeSql("SELECT count FROM publish_ledger WHERE ledger_date = ? AND asset_class = ?");
const SQL_UPSERT_LEDGER = normalizeSql("INSERT INTO publish_ledger (ledger_date, asset_class, count, last_updated) VALUES (@ledger_date, @asset_class, @count, strftime('%s','now')) ON CONFLICT(ledger_date, asset_class) DO UPDATE SET count = excluded.count, last_updated = strftime('%s','now')");
const SQL_SELECT_LOCK = normalizeSql("SELECT expires_at FROM locks WHERE name = ?");
const SQL_UPSERT_LOCK = normalizeSql("INSERT INTO locks(name, expires_at) VALUES(?, ?) ON CONFLICT(name) DO UPDATE SET expires_at = excluded.expires_at");
...
class MemoryDB {
    users = new Map();
    signals = new Map();
    signalByUniq = new Map();
    publishQueue = new Map();
    publishQueueByKey = new Map();
    dataSnapshots = new Map();
    locks = new Map();
    signalMetrics = new Map();
    publishLedger = new Map();
    nextSignalId = 1;
    nextQueueId = 1;
    unhandled = new Set();
    prepare(sql) {
    ...
    executeAll(sql, args) {
        switch (sql) {
            ...
            case SQL_POSTED_TODAY: {
                ...
                for (const row of this.publishQueue.values()) {
                    if (row.sent_at === null || (row.sent_at ?? 0) < since)
                        continue;
                    const signal = this.signals.get(row.signal_id);
                    if (!signal)
                        continue;
                    if (!counts.has(signal.asset_type))
                        counts.set(signal.asset_type, new Set());
...
if (!sqlitePath) {
    ...
    try {
        ...
        const DatabaseCtor = module.default ?? module;
