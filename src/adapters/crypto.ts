...
export async function getWhaleEvents(asset:string): Promise<WhaleEvent[]> {
  ...
  try {
    ...
    const events: WhaleEvent[] = txs.map((tx: any): WhaleEvent=>{
      const direction: 'inflow' | 'outflow' = tx.to?.owner_type === 'exchange' ? 'inflow' : 'outflow';
      return {
    ...
    return events.filter(tx=>Number.isFinite(tx.ts));
  } catch (err) {
